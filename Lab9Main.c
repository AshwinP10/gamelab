...
sprite_t asteroid = {
     ...
     0, // Random x coordinate
     0, // Random y coordinate
     {Asteroid, AsteroidExplode, AsteroidGone}, // Image array
     alive // Initial Status
...
// game engine goes here
    ...
    if (asteroid.x <= 5) // If asteroid is on edge, change the respective velocity
    {
        asteroidMotionX = 0.5;
    }
    if (asteroid.x >= 115)
    ...
    asteroid.x += asteroidMotionX; // Update new asteroid locations
    asteroid.y += asteroidMotionY;
...
int main(void){ // final main
  ...
  //Asteroid:
  asteroid.x = randomFloat(5, 115); // Initialize x coordinate
  asteroid.y = randomFloat(5, 155); // Initialize y coordinate
  asteroid.life = alive; // Initialize status
...
*/
          ...
          ST7735_DrawBitmap(asteroid.x, asteroid.y, Asteroid, 15,15);
