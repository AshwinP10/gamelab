...
void LED_Init(void){
    ...
    GPIOB->DOE31_0 |= 0x00180000; // enable DOE
    GPIOA->DOE31_0 |= 0x00020000; // enable DOE
...
void LED_On(uint32_t data){
    ...
    if (data == 1) // Turn on red
    {
        ...
        GPIOA->DOUTSET31_0 |= redLight;
    }
    if (data == 2) // Turn on yellow
    {
        GPIOB->DOUTSET31_0 |= yellowLight;
    }
    if (data == 3) // Turn on green
    {
        GPIOB->DOUTSET31_0 |= greenLight;
    }
...
void LED_Off(uint32_t data){
    ...
    {
        GPIOA->DOUTCLR31_0 = (GPIOA->DOUT31_0 & (~redLight));
    }
    if (data == 2) // Turn off yellow
    {
        GPIOB->DOUTCLR31_0 = (GPIOB->DOUT31_0 & (~yellowLight));
    }
    if (data == 2) // Turn off green
    {
        GPIOB->DOUTCLR31_0 = (GPIOB->DOUT31_0 & (~greenLight));
    }
